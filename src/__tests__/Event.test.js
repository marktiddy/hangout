...
describe("<Event /> component", () => {
  ...
  let EventWrapper = shallow(<Event event={dummyData[0]} />);

  test("check we have an event list", () => {
    expect(EventWrapper.find(".eventOverview")).toHaveLength(1);
  });
  test("details is collapsed by default", () => {
    expect(EventWrapper.find(".eventDetails")).toHaveLength(0);
  });
  test("user can click details button to expand info", () => {
